...
class crud
{
	...
	public function getFrequentAll($order) {
		$order = $order - 1;
		$feedAll = [];
		$stmt = $this->db->prepare("SELECT * FROM feedinfo WHERE frequentgenerate = '1' AND id MOD 10 = :remain AND id != '681' AND id !='381' AND id !='55' AND id != '377' AND id != '750' AND id != '873' AND id !='948' AND id !='955'");
		$stmt->bindparam(":remain",$order);
		$stmt->execute();
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			$feedAll[] = $row;
	...
	public function update($id,$name,$updatetag,$xmlurl,$defaultcountry,$industry,$company,$joblocationtype, $utmValue)
	{
		$now = new DateTime();
		$updateDate = $now->format('Y-m-d H:i:s');
		try
		{
			$newtag = "";
			$isnew = 0;
			$stmt=$this->db->prepare("UPDATE feedinfo SET name=:fname, updatetag=:updatetag, updatedate=:updatedate, url=:xmlurl, defaultcountry=:defaultcountry, industry=:industry, company=:company, joblocationtype=:joblocationtype, utm=:utm, newtag=:newtag, isnew=:isnew WHERE id=:id ");
